...
public interface PostRepository extends JpaRepository<Post, Integer> {
    ...
    @Query(
            value = "SELECT * FROM post WHERE author = :authorId",
            nativeQuery = true
    )
    List<Post> findByAuthor(@Param("authorId") int Author);
    ...
    @Query(value = "SELECT * FROM post WHERE is_published = true ORDER BY published_at DESC Limit :limit OFFSET :start",
           nativeQuery = true
          )    
     List<Post> findPostOrderByPublisedAtDesc(@Param("start") int start, @Param("limit") int limit);

    @Query(value = "SELECT * FROM post WHERE is_published = true ORDER BY published_at ASC Limit :limit OFFSET :start",
           nativeQuery = true
          ) 
